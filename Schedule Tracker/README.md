...
```bash
Schedule Tracker$ sam build --use-container
```
...
```bash
Schedule Tracker$ sam local invoke HelloWorldFunction --event events/event.json
```
...
```bash
Schedule Tracker$ sam logs -n HelloWorldFunction --stack-name Schedule Tracker --tail
```
...
```bash
Schedule Tracker$ pip install pytest pytest-mock --user
Schedule Tracker$ python -m pytest tests/ -v
```
...
```bash
aws cloudformation delete-stack --stack-name Schedule Tracker
```
